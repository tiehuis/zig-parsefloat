...
pub fn convertSlow(comptime T: type, s: []const u8) BiasedFp(T) {
    ...
    const BigDecimal = Decimal(T);
    const BiasedFp_ = BiasedFp(T);

    var d = BigDecimal.parse(s); // no need to recheck underscores
    if (d.num_digits == 0 or d.decimal_point < BigDecimal.min_exponent) {
        return BiasedFp_.zero();
    } else if (d.decimal_point >= BigDecimal.max_exponent) {
    ...
    while (d.decimal_point > 0) {
        ...
        d.rightShift(shift);
        if (d.decimal_point < -BigDecimal.decimal_point_range) {
            return BiasedFp_.zero();
        }
    ...
    if (mantissa >= (@as(MT, 1) << (mantissa_explicit_bits + 1))) {
    ...
    if (mantissa < (@as(MT, 1) << mantissa_explicit_bits)) {
        power2 -= 1;
    }
    // Zero out all the bits above the explicit mantissa bits.
    mantissa &= (@as(MT, 1) << mantissa_explicit_bits) - 1;
