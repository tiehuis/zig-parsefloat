...
pub fn convertSlow(comptime T: type, s: []const u8) BiasedFp {
    ...
    if (mantissa >= (@as(u64, 1) << (mantissa_explicit_bits + 1))) {
    ...
    if (mantissa < (@as(u64, 1) << mantissa_explicit_bits)) {
        power2 -= 1;
    }
    // Zero out all the bits above the explicit mantissa bits.
    mantissa &= (@as(u64, 1) << mantissa_explicit_bits) - 1;
